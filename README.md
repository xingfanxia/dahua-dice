...
A 2-8 player Liar's Dice web app with animated 3D cube dice, gyroscope shake-to-reveal, a solo dice-cup and a vs-computer mode, in a clean neutral-gray + red design language (shared with the WeChat mini-program sibling). Mobile-first, realtime, server-authoritative.
...
- 🎲 **3D cube dice** — pure CSS 3D cubes (perspective + `preserve-3d`, six pip faces, transition-driven spin-and-land), no WebGL/Three.js: `components/dice/DiceCube` is the per-die cube and `components/dice/MyHand` is the player's own hand with the unified tap/shake gesture; the inline dice in the reveal / bid / chain share a small SVG `PipDie` (no emoji glyphs). Reduced-motion aware, Tailwind light/dark. The roll is always server-authoritative (`crypto.randomInt`); the animation is decorative.
- 📱 **Mobile-first** with gyroscope shake-to-reveal (DeviceMotion API, iOS permission flow), `100dvh` safe-area layout, native share-sheet invite links.
